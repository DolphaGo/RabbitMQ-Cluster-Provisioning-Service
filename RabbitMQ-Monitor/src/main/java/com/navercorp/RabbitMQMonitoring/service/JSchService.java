...
import com.navercorp.RabbitMQMonitoring.util.jsch.JSchUtil;
...
public class JSchService {
    ...
    /**
     * node의 hosts file에 addList의 정보를 추가함
     */
    public void editHosts(Node targetNode,List<Node> addList) throws Exception{
        String dir = "/etc/hosts";

        Session session = jschUtil.createSession(targetNode);
        Channel channel = session.openChannel("sftp");
        ChannelSftp sftp = (ChannelSftp) channel;
        sftp.connect();
